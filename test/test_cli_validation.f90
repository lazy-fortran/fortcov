program test_cli_validation
    ...
    write(output_unit, '(A)') "======================================================="
    write(output_unit, '(A)') "             CLI Validation Test Suite                 "
    write(output_unit, '(A)') "======================================================="
    write(output_unit, '(A)') ""
    ...
    write(output_unit, '(A)') ""
    write(output_unit, '(A)') "======================================================="
    write(*, '(A,I0,A,I0,A)') "CLI VALIDATION: ", passed_tests, "/", &
                              test_count, " tests passed"

    if (all_tests_passed) then
        write(output_unit, '(A)') "✅ ALL CLI VALIDATION TESTS PASSED"
        call exit(0)
    else
        write(output_unit, '(A)') "❌ CLI VALIDATION TESTS FAILED"
        call exit(1)
    end if
