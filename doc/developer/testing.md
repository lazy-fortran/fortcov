...
- CI runs an end-to-end validation that compares FortCov’s total coverage to a
  Cobertura XML generated by `gcovr`. See workflow `.github/workflows/e2e_pycobertura.yml`.
- Comparator script: `scripts/compare_coverage_metrics.py`.
- The check fails if totals differ beyond a tiny tolerance and rejects empty or malformed reports.
