...
- Run the full suite with `fpm test` (use `--verbose` when diagnosing).
- Keep tests deterministic and isolated; clean up temporary files.
- Prefer direct module testing in Fortran tests; add targeted integration tests where valuable.
- Use clear assertions and exit with non-zero status on failure.
