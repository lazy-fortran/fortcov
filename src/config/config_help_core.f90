...
contains
    ...
    subroutine show_help_information()
        ...
        print '(A)', "  -o, --output PATH         Output file path (format auto-detected from extension)"
        print '(A)', "  -f, --format FORMAT       Output format (overrides auto-detection):"
        ...
        print '(A)', "  fortcov --source=src *.gcov --output=report.md   # Generates markdown (auto-detected)"
        print '(A)', "  fortcov --source=src *.gcov --output=coverage.html # Generates HTML (auto-detected)"
        print '(A)', "  fortcov --source=. *.gcov --format=json        # JSON output format (explicit)"
